...
- **Export clean markdown** — the inverse: export a line range (or the whole note) as portable markdown via the `gy` operator (`gyap`, `gy3j`, `gy` on a visual selection; `:FtExport` exports the whole note). Before copying it asks which format: `commonmark` (portable CommonMark) or `slack` (Slack mrkdwn — headings → bold, `[t](u)` → `<u|t>`, checkboxes and callout markers dropped); set `export.format` to skip the prompt. Frontmatter, `[!ft-source]` callout headers, and wikilinks are stripped by `ft notes export` — ready to paste into other apps, chats, or notes without vault provenance. Same registers as quote (they share register `f`, last op wins)
...
- `:FtExport` — export the whole note (normal mode) or the selection (visual mode) as clean CommonMark; an explicit range (`:3,6FtExport`) exports those lines (`gy` + a motion does the same as an operator). Asks for the format (`commonmark`/`slack`) unless `export.format` is set
