...
error), pinned by the quote stub tests. Export's failures classify on
the message text too (`outside file` → warn, everything else → error),
...
| Range-op glue | One shared `ft.rangeop` core (preflight, save, classify, register placement) behind quote + export | Per-operation copies of the ~80-line mechanics — register semantics drift between ops |
