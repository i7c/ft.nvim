...
- [ ] 1.1 In `lua/ft/export.lua`, add the hardcoded format list
      `{ { id = 'commonmark', desc = <CLI desc> }, { id = 'slack',
      desc = <CLI desc> } }` and a `known[format]` lookup for config
      validation.
- [ ] 1.2 Add a `resolve_format(cfg, cb)` step: when `export.format`
...
- [ ] 1.3 Thread the resolved format into both `rangeop.run` call
...
- [ ] 1.4 Update the success notifications to name the format:
      `'ft: exported L' .. s .. ' (' .. fmt .. ') → ' …` and
      `'ft: exported whole note (' .. fmt .. ') → ' …`.
- [ ] 1.5 Add `format = 'prompt'` to `defaults.export` in
      `lua/ft/init.lua` and update the setup docstring.

## 2. Version floor

- [ ] 2.1 Bump `MIN_FT_VERSION` from `{0,1,5}` to `{0,1,7}` in
...
- [ ] 2.2 Update the `ft notes export` row of the protocol contract
